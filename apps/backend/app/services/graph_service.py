...
class GraphService:
    ...
    async def initialise(self) -> None:
        ...
        try:
            ...
            logger.warning("Graph initialisation failed — routing will use mock fallback: %s", exc)
            self._graph = None
            self._node_index = []
    ...
    async def _build_graph(self) -> None:
        ...
        for route in routes:
            ...
            valid_route_stops = [
                rs for rs in route_stops if rs.stop.latitude != 0.0 and rs.stop.longitude != 0.0
            ]
        ...
        for tp in transfers:
            ...
            from_stops = [
                rs.stop
                for rs in tp.from_route.route_stops
                if rs.stop.latitude != 0.0 and rs.stop.longitude != 0.0
            ]
            to_stops = [
                rs.stop
                for rs in tp.to_route.route_stops
                if rs.stop.latitude != 0.0 and rs.stop.longitude != 0.0
            ]
            ...
            from_stop = next(
                (stop for stop in from_stops if stop.stop_name == tp_stop_name),
                None,
            )
            to_stop = next(
                (stop for stop in to_stops if stop.stop_name == tp_stop_name),
                None,
            )
            if from_stop is None or to_stop is None:
                logger.warning(
                    "Skipping invalid explicit transfer '%s': named stop must "
                    "exist on both routes",
                    tp.name,
                )
                continue
            ...
            for src, tgt in [
                (
                    f"{from_stop.id}_{tp.from_route_id}",
                    f"{to_stop.id}_{tp.to_route_id}",
                ),
                (
                    f"{to_stop.id}_{tp.to_route_id}",
                    f"{from_stop.id}_{tp.from_route_id}",
            ...
            logger.info(
                ...
                tp.from_route.route_name,
                tp.to_route.route_name,
                tp_stop_name,
                dist,
            )
        ...
        logger.info(
            "Automatically generating transfer edges between routes (max 150m walking radius)..."
        )
    ...
    async def _load_transfers(session: Any) -> list[TransferPoint]:
        ...
        stmt = select(TransferPoint).options(
